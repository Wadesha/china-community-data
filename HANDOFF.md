...
> 本文档面向后续接力的模型或开发者。读完即可独立完成:补数据、换真实数据上线、维护六个页面、续写三部百科卷、发布到 GitHub Pages。**先读 §8 项目哲学再动手**,那是本项目一切取舍的依据。
...
- **零依赖纯静态**:无框架、无构建步骤、无 node_modules。六个 HTML + 一个 JSON,`python3 -m http.server` 即可本地预览。
- **history.html / planning.html / landscape.html 完全自包含**:不读 JSON、无外部资源,单文件可独立打开。planning.html、landscape.html 与 history.html 同体例(条目/定义段/技术档案/图版/参见),但**编号独立**——三卷的"图 n-n""档 n-n"互不相涉,校验脚本按文件分开跑。
...
## 4. 百科卷:体例与编号系统(history.html / planning.html / landscape.html)

三卷均为单文件自包含百科体例:凡例 → 总纲(分期框架) → 六编 → 四附录(A 术语表 / B 参考来源 / C 总年表 / D 图版与档案目录)。卷一《中国居住建筑史》(177KB)回答"房子长什么样";卷二《中国城市规划与区域发展》(122KB,先秦—2026)回答"城市怎么被组织起来",以规划权归属/编制依据/规划对象三项制度变量断代;卷三《中国近现代城乡风貌》(125KB,1840—2026)回答"眼前这块风貌是什么时候、被谁、按什么规则造出来的",以建造主体/资金来源/规则依据三变量整体换代分四期,城市乡村双线并观,第六编与附录为识别工具(建筑年代五维鉴定、街道剖面法)。维护前必读:

### 4.1 编号系统(增量维护的核心约束)

**三卷编号完全独立**:卷二的"图 1-1"与卷一的"图 1-1"互不相涉,卷三亦然。跨卷引用写明"→卷一 档 3-2"这类带卷号的形式,不能用裸编号。

| 编号 | 格式 | 卷一 history.html 分配 | 卷二 planning.html 分配 | 卷三 landscape.html 分配 |
|---|---|---|---|---|
| 图版 | `图 n-n`(n=编次) | 图 1-1…1-2, 2-1, 3-1…3-2, 5-1…5-4,共 9 幅(第四编无图版) | 图 1-1…1-2, 2-1, 3-1, 4-1, 5-1…5-2, 6-1…6-2,共 9 幅 | 图 1-1…1-2, 2-1, 3-1…3-2, 4-1, 5-1, 6-1…6-2,共 9 幅 |
| 技术档案 | `档 n-n` | 档 1-1…1-4, 2-1, 3-1…3-2, 6-1…6-7,共 14 卷(第五编无档案) | 档 1-1, 3-1, 4-1, 5-1…5-2, 6-1,共 6 卷(第二编无档案) | 档 1-1, 2-1, 3-1…3-2, 4-1, 5-1, 6-1,共 6 卷 |
| 正文引用 | `[n]` | 1–65 连续 | 1–11 连续(制度文本与通史著作为纲,多数无 URL) | 1–12 连续(保护名录、村落名录、普查统计、更新政策为纲) |

硬规则:**每条附录 B 来源必须有正文上标对应,不允许"只列不用"**。已验证:卷一 65/65、卷二 11/11、卷三 12/12 全部被引用。

### 4.2 术语表(附录 A)

按拼音序插入(`<div class="lemma">`),新词条须带交叉引用(→图/档/§)。三卷各 32 条。

### 4.3 目录(左侧 nav.toc)

新章节需同步:导航 `<a href="#id">` + 正文 `<div class="sec" id="...">` 或 `<div class="vol" id="...">`。滚动高亮脚本按 id 匹配。

### 4.4 修改后必跑的校验(三个都要过,三卷各跑一遍)

```bash
cd /Users/wade/Library/CloudStorage/OneDrive-个人/claw/anjuke/site
# 把下面三段里的 history.html 换成 planning.html / landscape.html 再各跑一遍即为卷二/卷三校验
...
- [x] 站点六页面完成,模拟数据(10 省/32 城/3798 小区)
...
| 新页面 | 复制任一现有页的 header/topnav 结构 → topnav 六页互链同步更新 → README 表格补行 |
